...
# ===-===-
function averageLandscpceInDiscreteSetOfPoints(
    ...
    if to <= from
        ...
        errMessage <<
        "Error in function averageLandscpceInDiscreteSetOfPoints. You want to compute average inrval [$(from),$(to)]. Please correct the vinterval in a way that it is in a form [from,to], where from <to"
        throw(errMessage)
    end
    ...
    for i = 1:size(v_space_pland.vectOfLand, 1)
        ...
        if dim(v_space_pland.vectOfLand[i]) >= dim
            ...
            while (
                (nr != numberOfPoints) &&
                (it != size(v_space_pland.vectOfLand[i].land[dim], 1))
            )

                if local_dbg
                    println(
                        "nr : $(nr) , it : $(it) , numberOfPoints : $(numberOfPoints) , v_space_pland.vectOfLand[i].land[dim].size() : $(2)",
                    )
...
function standardDeviation(
    ...
    for i = 1:size(v_space_pland.vectOfLand, 1)
        ...
        elseif whichDistance == "maxNorm"
            #L^infty distance
            distance =
                computeMaxNormDiscanceOfLandscapes(av, v_space_pland.vectOfLand[i], 0)
        end
